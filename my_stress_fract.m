function [stran,stres] = my_stress_fract(asdis,nelem,npoin,nnode,ngaus,nstre,props,ntype,ndofn,ndime,lnods,matno,coord,posgp,weigp,dgdx,dvolum,tdisp)
format long;
...
for igaus = 1 : ngaus
    ...
    for jgaus = 1 : ngaus2
        ...
        for istre = 1 : nstre
            for ievab = 1 : nevab
                stran( :, kgasp, istre) = stran( :, kgasp, istre) + ...
                    bmatx( :, istre, ievab) .* eldis( :, ievab);
            end
        end
        ...
        for istre = 1 : nstre
            for jstre = 1 : nstre
                stres( :, kgasp, istre) = stres( :, kgasp, istre) + ...
                    dmatx(istre,jstre) .* stran( :, kgasp, jstre);
            end
        end
    end %igaus
end %jgaus
